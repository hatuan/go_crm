...
func PostRatingsWithHeaderID(headerID int64, ratings []Rating) ([]Rating, TransactionalInformation) {
	validationErrors := make(map[string]InterfaceArray)
	_ids := []int64{}
	...
	if len(_ids) != 0 {
		query, args, err := sqlx.In("DELETE FROM rating WHERE profile_questionnaire_header_id = ? AND id NOT IN (?)", headerID, _ids)
		if err != nil {
			tx.Rollback()
			return []Rating{}, TransactionalInformation{ReturnStatus: false, ReturnMessage: []string{err.Error()}}
		}
	...
	ratings, _ = GetRatingsByHeaderID(headerID)
